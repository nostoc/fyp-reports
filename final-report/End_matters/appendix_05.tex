...
This appendix lists every automated test case referenced in Chapter~\ref{ch:testing}, together with its identifier where the source file defines one, so that the full test inventory remains traceable without interrupting the discussion in the main chapter. The recorded pass/fail outcome and aggregate timing for each suite is presented in Chapter~\ref{ch:testing}; this appendix focuses on what each individual case verifies. Screenshots of the running system captured during testing are also collected here.
...
\subsection{\texttt{EmergencyReporting.ts}}
...
\hline
\endhead
T6.1 & All 100 duplicate submission-nullifier replay attempts are rejected \\
\hline
T6.2 & A non-relayer account calling \texttt{submitReport} directly is rejected as \texttt{Unauthorized} \\
\hline
T6.3 & A second validation vote from the same citizen pseudonym is rejected even with a fresh vote nullifier \\
\hline
T6.4 & A second poll vote with the same nullifier is rejected \\
\hline
T6.5a & A penalized citizen is blocked from submitting another emergency report for 30 days \\
\hline
T6.5b & A penalized citizen can submit again once the 30-day penalty has expired \\
\hline
T6.6 & A second verification vote from the same citizen pseudonym is rejected \\
\hline
\end{longtable}

\subsection{\texttt{OpinionPolling.ts}}
...
\hline
\endhead
An authorized authority can create a poll \\
\hline
Poll creation by a non-authority is reverted \\
\hline
Poll creation with a deadline in the past or present is reverted \\
\hline
Votes are recorded correctly and \texttt{VoteCast} is emitted \\
\hline
Double voting with the same nullifier is reverted \\
\hline
Voting after the poll deadline is reverted \\
\hline
Voting on a finalized or deactivated poll is reverted \\
\hline
Finalizing a poll deactivates it and emits \texttt{PollFinalized} \\
\hline
Finalizing a poll that does not exist is reverted \\
\hline
Finalizing a poll that is already inactive is reverted \\
\hline
\end{longtable}

\subsection{\texttt{AuthorityMultiSig.ts}}
...
\hline
\endhead
Super Admins can add a new Super Admin through majority vote \\
\hline
Super Admins can remove a Super Admin \\
\hline
Super Admins can add an authority worker \\
\hline
Super Admins can remove an authority worker \\
\hline
\end{longtable}
...
\begin{longtable}{p{0.14\textwidth}|p{0.86\textwidth}}
\caption{Relayer unit test cases} \\
\hline
  {ID} &   {Purpose} \\
\hline
\endfirsthead
...
\hline
\endhead
\multicolumn{2}{l}{\textit{app.controller.spec.ts}} \\
\hline
--- & Application bootstrap smoke test (root controller returns a response) \\
\hline
\multicolumn{2}{l}{\textit{ai-oracle.service.spec.ts}} \\
\hline
T2.5.1 & \texttt{canonicalJson()} produces deterministic, sorted output regardless of key order \\
\hline
T2.5.2 & The relayer signature can be recovered and verified by ethers.js \\
\hline
T2.5.3 & A missing \texttt{RELAYER\_PRIVATE\_KEY} logs a warning and fails safely \\
\hline
\multicolumn{2}{l}{\textit{reporting.service.spec.ts}} \\
\hline
T2.1.1 & A valid government signature is accepted and returns a job ID \\
\hline
T2.1.2 & A wrong government signer address is rejected with \texttt{UnauthorizedException} \\
\hline
T2.1.3 & A missing \texttt{zkpSignature} field throws \texttt{BadRequestException} \\
\hline
T2.2.1 & A valid citizen signature is accepted \\
\hline
T2.2.2 & A tampered description causes a signature mismatch, throwing \texttt{UnauthorizedException} \\
\hline
T2.2.3 & A wrong \texttt{citizenPubKey} (address mismatch) causes rejection \\
\hline
T2.2.4 & An image count mismatch throws \texttt{BadRequestException} \\
\hline
T2.2.5 & Malformed \texttt{imageHashes} JSON throws \texttt{BadRequestException} \\
\hline
...
\hline
T2.4.1 & A valid vote payload returns a job ID \\
\hline
T2.4.2 & An invalid government ticket for a vote throws \texttt{UnauthorizedException} \\
\hline
T2.4.3 & A tampered vote citizen signature throws \texttt{UnauthorizedException} \\
\hline
T2.4.4 & Missing required vote fields throw \texttt{BadRequestException} \\
\hline
T2.5.1 & A missing \texttt{GOV\_PUBLIC\_ADDRESS} throws on module initialization \\
\hline
\end{longtable}
...
\hline
\endhead
\multicolumn{2}{l}{\textit{test\_api\_security.py}} \\
\hline
...
\hline
\multicolumn{2}{l}{\textit{test\_classification.py}} \\
\hline
...
\begin{longtable}{p{0.14\textwidth}|p{0.86\textwidth}}
\caption{Blockchain network performance benchmark test cases (\texttt{benchmark.ts})} \\
\hline
  {ID} &   {Purpose} \\
\hline
\endfirsthead
